...
Np = 4
np.random.seed(1234)
f = np.random.randint(20, size = (I))*10 # boshen gives an array sink = 0
...
c = np.random.randint(10,size=(I,I)) # boshen gives cost matrix instead of random [I*I matrix] cost = distance [row to column] very big if not connected
for i in range(I):
    c[i,i] = 0
c_p_1 = 100
c_p_2 = 50
...
cp = np.random.randint(sum(f)/2,size=(I,I)) # boshen gives capacity matrix instead of random I*I matrix capacity = width 0 if not connected
for i in range(I):
    cp[i,i] = 0
print(cp)
...
decision_input= np.random.randint(1,size=(I,I))

decision_input

# read the decision from graph; boshen corrects the numbers inside []
decision_input[0,2] = 1
decision_input[2,3] = 1
decision_input[1,3] = 1
decision_input[3,4] = 1
...
for i in range(I):
  for j in range(I):
    # model.bound.add(model.z[i,j] == model.x[i,j] * model.D[i,j]) #3b
    model.bound.add(model.z_a[i,j] >= 0) #3c
    model.bound.add(model.z_a[i,j] >= model.y_a[i,j] + decision_input[i,j] * big_m_1 - big_m_1) #3d
    model.bound.add(model.z_a[i,j] <= model.y_a[i,j]) #3e
    model.bound.add(model.z_a[i,j] <= decision_input[i,j] * big_m_1) #3f 
    model.bound.add(model.z_t[i,j] >= 0) #3c
    model.bound.add(model.z_t[i,j] >= model.y_t[i,j] + decision_input[i,j] * big_m_1 - big_m_1) #3d
    model.bound.add(model.z_t[i,j] <= model.y_t[i,j]) #3e
    model.bound.add(model.z_t[i,j] <= decision_input[i,j] * big_m_1) #3f 

    model.bound.add(model.alpha[i,j] + model.beta[i] -  model.beta[j]  <= c[i,j] + c_p_1*decision_input[i,j]) #3i
    model.bound.add(model.alpha[i,j] + model.tau[i] -  model.tau[j]  <= c[i,j] + c_p_2*decision_input[i,j]) #3i
    model.bound.add(model.alpha[i,j] + model.gamma[i] -  model.gamma[j]  <= c[i,j] ) #3i
...
# Calculate Fraction
fraction = user_result/best_result
